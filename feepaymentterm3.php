...
<div><a style="float:right" href="logout.php"><button>Log out</button></a></div><br/></br>
      <div> <?php
        ...
        <?php
        $sql="SELECT users.id,payment.payment_id,users.firstname,users.myclass, users.lastname, payment.payment_date, payment.amount
        FROM users
        INNER JOIN payment
        ON users.id = payment.student_id
        where payment.term_id=3
        order by amount;";

$result = mysqli_query($conn, $sql);
if (mysqli_num_rows($result) > 0) {
 echo "<table> ";
 echo "<h2>Term Three</h2>";
 echo "<thead> <tr>";
